...
# Fit boosted tree model with feature engineering recipe
...
bt_params <- extract_parameter_set_dials(bt_spec) |>
  update(mtry = mtry(range = c(1,50))) #max 36 bc it's 70% of total predictors
...
# tune ----
tuned_bt_fe <- tune_grid(bt_workflow_fe, 
                            articles_fold,
                            grid = bt_grid,
